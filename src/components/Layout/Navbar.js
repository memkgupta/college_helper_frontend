  ...
  return (
    ...
    <div class="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
      ...
      <nav class="md:ml-auto md:mr-auto flex flex-wrap items-center text-base justify-center">
        ...
        <Link to={'/chat-room'} className={`mr-5  ${theme==='dark'?'dark:text-white':''} hover:${theme==='dark'?'bg-blue':'text-gray'}`}>Discuss</Link>
      </nav>
      <Link  to={user._id?"/account":'/Auth'} class="inline-flex items-center text-white bg-gray border-0 py-1 px-3 focus:outline-none hover:bg-gray-200 rounded text-base mt-4 md:mt-0">{user.name?'Account':'Login'}
